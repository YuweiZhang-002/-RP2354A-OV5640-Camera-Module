...
/* Define the struct format for writing sensors  */
typedef struct{
	uint16_t reg;
	uint16_t value;
}sensor_reg_t;

typedef sensor_reg_t ov5640_reg_t;


/*
 * --------------------------------------------------------------------------
 * OV5640 initialization sequence — split into functional modules
 * --------------------------------------------------------------------------
 *
 * Rules followed by this split:
 *   1. Register set is identical to the legacy OV5640_Init[] array.
 *   2. Register values are byte-for-byte unchanged.
 *   3. No register is removed; no register is reordered.
 *   4. Each functional module is an independent array.
 *   5. Applying the modules top-to-bottom (in the order they appear here)
 *      reproduces the original write sequence exactly. The contiguous
 *      slicing is required because some OmniVision analog tweaks are
 *      interleaved with AEC writes in the vendor-recommended sequence.
 *
 * Module order (apply in this order):
 *   01 reset                 -> 02 analog_tuning_1     -> 03 aec_ceiling
 *   04 analog_tuning_2       -> 05 banding_lightmeter  -> 06 timing
 *   07 analog_tuning_3       -> 08 aec_step_expo       -> 09 blc
 *   10 clock_mipi            -> 11 format              -> 12 pclk
 *   13 isp_top               -> 14 awb                 -> 15 cmx
 *   16 cip                   -> 17 gamma               -> 18 sde
 *   19 lenc                  -> 20 aec_final           -> 21 wakeup
 *   (From here on, the modules are supplementary, applied for changing some critical definitions)
 *   22 dvp                   -> 23 resolution          -> 24 pixel_format
 *   25 polarity              -> 26 power_on
 *
 * Module 22 is supplementary: it ports the register writes performed by
 * OV5640_EnableDVPMode() in ov5640_cmp.c that are absent from the legacy
 * OV5640_Init[] array, and additionally hard-gates the MIPI PHY (this
 * project never uses the MIPI serial output).
 * --------------------------------------------------------------------------
 */

/* 00-21) OV5640 Common Initialization Sequence
 *
 *     Canonical sequence extracted from OV5640_Common[] in ov5640_cmp.c.
 *     All 246 registers are preserved in exact order and with exact values
 *     as provided by the ST vendor driver. This unified array replaces the
 *     previously distributed module arrays (01-21) to ensure compatibility
 *     with the original OV5640_Init() flow from ov5640_cmp.c.
 *
 * Module structure (preserved for reference):
 *   01 reset + 02 analog_tuning_1  + 03 aec_ceiling
 *   04 analog_tuning_2             + 05 banding_lightmeter + 06 timing
 *   07 analog_tuning_3             + 08 aec_step_expo + 09 blc
 *   10 clock_mipi                  + 11 format + 12 pclk
 ...
 *   19 lenc                        + 20 aec_final + 21 wakeup
 */
static const sensor_reg_t ov5640_init_common[] = {
    ...
    {OV5640_TIMING_HS_LOW,      0x98},   // 改:0x02A0(672)→0x0298(664),窗口放大到 1296 宽后居中的新起点
    {OV5640_TIMING_VS_HIGH,     0x01},
    {OV5640_TIMING_VS_LOW,      0xE8},   // 改:0x01F6(502)→0x01E8(488),窗口放大到 968 高后居中的新起点
    {OV5640_TIMING_HW_HIGH,     0x07},
    {OV5640_TIMING_HW_LOW,      0xA7},   // 改:0x079F(1951)→0x07A7(1959),对应 1296 宽
    {OV5640_TIMING_VH_HIGH,     0x05},
    {OV5640_TIMING_VH_LOW,      0xAF},   // 改:0x05B5(1461)→0x05AF(1455),对应 968 高
...
/* 22) DVP mode (No MIPI)
 *
 *     Registers that ov5640_cmp.c writes inside OV5640_EnableDVPMode() but
 *     that the legacy OV5640_Init[] sequence never touched. This module
 *     also forces the MIPI PHY off: this project drives the parallel DVP
 *     interface only and does not implement the MIPI serial path.
 *
 *     Already-present-elsewhere registers (0x300E MIPI_CONTROL00=0x58,
 *     0x302e=0x00, 0x471c=0x50) are not re-written here to avoid
 *     duplicate I2C transactions — they are already programmed by
 *     ov5640_init_clock_mipi[] / ov5640_init_analog_tuning_1[].
 *
 *     MIPI_CTRL00 (0x4800) = 0x64 :
 *         bit[6] clk_lane_disable      = 1  (gate MIPI clock lane)
 *         bit[5] clk_lane_gating_en    = 1  (enable clock-lane gating)
 *         bit[2] lp_idle_min           = 1  (minimise lane idle window)
 *       Net effect: MIPI PHY is parked in LP-11 idle / output disabled.
 */
static const sensor_reg_t ov5640_init_dvp[] = {
...
/* 24) Pixel Format Adjustment
 *     Registers that adjust the output pixel format. 
 *     Note: The base format in ov5640_init_format[] is RGB565, 
 *     The per-format tables below can be applied to switch to the desired format.
 */
/* Initialization sequence for RGB565 pixel format */
  static const sensor_reg_t ov5640_PF_RGB565[] =
  {
  ...
  static const sensor_reg_t ov5640_PF_YUV422[] =
  {
  ...
  static const sensor_reg_t ov5640_PF_RGB888[] =
  {
  ...
  static const sensor_reg_t ov5640_PF_Y8[] =
  {
  ...
  static const sensor_reg_t ov5640_PF_JPEG[] =
  {
...
/* 25) Polarity Adjustment
 *     Registers that adjust the PCLK, VSYNC and HREF polarity. 
 *     Note: The polarity in setup stage is VSYNC/HREF active high,
 */
static const sensor_reg_t ov5640_polarity_regs[] = {
    /* Polarity adjustment: All signals active low   */
    {OV5640_POLARITY_CTRL, 0x00},
    /* Polarity adjustment: VSYNC active high, HREF/PCLK active low */
    {OV5640_POLARITY_CTRL, 0x01},
    /* Polarity adjustment: VSYNC/HREF active high, PCLK active low */
    {OV5640_POLARITY_CTRL, 0x02},
    /* Polarity adjustment: HREF/VSYNC active low, PCLK active high */
    {OV5640_POLARITY_CTRL, 0x20},
    /* Polarity adjustment: HREF active low, VSYNC/PCLK active high */
    {OV5640_POLARITY_CTRL, 0x21},
    /* Polarity adjustment: All signals active high */
    {OV5640_POLARITY_CTRL, 0x22},
};


/* 26) Power Adjustment
 *     Registers that adjust the system's overall power state status. 
 */
static const sensor_reg_t ov5640_power_regs[] = {
    /* Power adjustment: Power on */
