...
| 126-127 | 2 | CRC 字段 | 当前占位值 `FF FF` |

offset 9 的 bit 2 不是第 0 行或帧起点标志。接收端应通过 `row_idx == 0` 判断首行；bit 2 标记第 2 行，即第一个具备完整三行 Sobel 窗口的处理行。

固件中已经存在 CRC-16/CCITT 计算函数，但当前数据包生成逻辑并未调用该函数。在显式启用 CRC 之前，offset 126-127 始终为 `0xFFFF`，只能视为占位符，不能视为 CRC 校验成功。RP2354 当前也会把 offset 13 写为零；如果 FPGA 后续需要替换该字节并写入状态，必须单独定义版本化语义并完成验证。
...
- 协议冻结后，在数据包生成端启用 CRC-16/CCITT，并定义接收端的 CRC 失败指示。
